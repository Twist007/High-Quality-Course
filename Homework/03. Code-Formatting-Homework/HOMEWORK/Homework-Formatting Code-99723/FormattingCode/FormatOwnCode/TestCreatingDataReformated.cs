...
namespace MiddleTierTest
{
    ...
    public static void TestCreatingData(int NumberOfEmployees, int NumberOfDependents)
    {
        Console.WriteLine("Creating " + NumberOfEmployees.ToString() + " employees each with "
    ...
    public static void TimeFunction(TestRoutine func, RWMode Mode, ObjectCreation ObjCreate, string EmpID,
                                    int Iterations)
    {
        GarbageCollect();
        ...
        Console.WriteLine(" T {0:F3} M {1:F3} X {2:F3} A {3:F3} GC {4:F3}", tot, min, max, avg, gc);
    }
}